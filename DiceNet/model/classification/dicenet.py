...
import sys
sys.path.insert(0, '/hdd1/home/bchaudhary/hyeirn/baseline/baseline/EdgeNets/')
...
class CNNModel(nn.Module):
    def __init__(self, args):
        ...
        width = int(width / 2)
        height = int(height / 2)
        self.level1 = CBR(channels_in, out_channel_map[0], 1, 1)
        width = int(width / 2)
        height = int(height / 2)
...
if __name__ == '__main__':
    ...
    for scale in config_all.sc_ch_dict.keys():
        for size in [600]:
            ...
            model = CNNModel(args)
            input = torch.randn(1, 60, size, size)
